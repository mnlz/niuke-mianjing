...
.\.venv\Scripts\python.exe -m niuke_mianjing_backend.crawler.recruitment.cli tencent --keyword 后端 --pages 1 --page-size 5
```
...
3. 在 `registry.py` 的 `ADAPTERS` 中注册。
4. 保留官网原始岗位链接和 `raw_data`，方便后续追溯及重新分析。
